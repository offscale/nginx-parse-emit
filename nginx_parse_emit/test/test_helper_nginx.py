...
class TestParseEmit(TestCase):
    def setUp(self):
        ...
        self.parsed_api_block = loads(api_proxy_block(location=self.location,
                                                      proxy_pass=self.proxy_pass))

        # server_block_no_rest
        self.server_name = 'offscale.io'
        self.listen = '443'
        self.parsed_server_block_no_rest = loads(server_block(server_name=self.server_name,
                                                              listen=self.listen))
    ...
    def test_add_redirect_and_cert(self):
        output0 = upsert_ssl_cert_to_443_block(upsert_redirect_to_443_block(self.one_root,
                                                                            self.server_name),
                                               self.server_name,
                                               self.ssl_certificate,
                                               self.ssl_certificate_key)
        output1 = upsert_ssl_cert_to_443_block(upsert_redirect_to_443_block(output0,
                                                                            self.server_name),
                                               self.server_name,
                                               self.ssl_certificate,
                                               self.ssl_certificate_key)
        for _ in xrange(5):
            output1 = upsert_ssl_cert_to_443_block(upsert_redirect_to_443_block(output1,
                                                                                self.server_name),
                                                   self.server_name,
                                                   self.ssl_certificate,
                                                   self.ssl_certificate_key)
        ...
        for term in self.server_name, '443', self.ssl_certificate, self.ssl_certificate_key:
            self.assertIn(term, output1_s)
    ...
    def test_proxy_add(self):
        ...
        for term in (self.proxy_pass, self.server_name, '443',
                     self.ssl_certificate, self.ssl_certificate_key,
